...
class NgramLanguageModel:
    ...
    def train(self, infile='samiam.train'):
        ...
        for s in range(len(sentences)):
            sentences[s] = '<s> ' + sentences[s] + ' </s>'
        #counting unigrams
        for i in range(len(sentences)):
            tokens = sentences[i].split()
            for i in range(len(tokens)):
                if self.unigram_counts[tokens[i]] == 0: self.unigram_counts[tokens[i]] = 1
                else: self.unigram_counts[tokens[i]] += 1

        #calculating vocab
        test = self.unigram_counts.keys()
        self.V = len(test)
        ...
        #counting bigrams
        for s in range(len(sentences)):
            tokens = sentences[s].split()
            for i in range(len(tokens)):
                if i != len(tokens) - 1:
                    if self.bigram_counts[tokens[i] + "_" + tokens[i+1]] == 0: self.bigram_counts[tokens[i] + "_" + tokens[i+1]] = 1
                    else: self.bigram_counts[tokens[i] + "_" + tokens[i+1]] += 1
    ...
    def predict_unigram(self, sentence):        
        ...
        for s in sen:
            if s != '<s>':
                c = self.unigram_counts[s]
                prob = (c + self.k) / (self.N + (self.k * self.V))
                senP += math.log(prob)
    ...
    def predict_bigram(self, sentence):
        ...
        for s in range(len(sen)):
            if s != len(sen) - 1:
                c = self.bigram_counts[sen[s] + "_" + sen[s+1]]

                #getting Wn-1
                w = self.unigram_counts[sen[s]]

                prob = (c + self.k) / (w + (self.k * self.V))
                senP += math.log(prob)
    ...
    def test_perplexity(self, test_file, ngram_size='unigram'):
        ...
        #separating the test corpus into sentences
        sentences = []
        for line in open(test_file).readlines():
            sentences.append(line)
        
        #calculating total probability of test corpus
        totalP = 0.0
        totalT = 0
        for s in sentences:
            if ngram_size == 'unigram':
                totalP += self.predict_unigram(s)
                t = s.split()
                totalT += len(t) + 1
            if ngram_size == 'bigram':
                totalP += self.predict_bigram(s)
                t = s.split()
                totalT += len(t) + 1


        perplexity = math.exp((totalP / totalT) * -1)
...
if __name__ == '__main__':
    ...
    ngram_lm.train('/Users/gustavolucasdecarvalho/Desktop/a2/samiam.train')
    print('Training perplexity, unigram:\t', ngram_lm.test_perplexity('/Users/gustavolucasdecarvalho/Desktop/a2/samiam.train', 'unigram'))
    print('Training perplexity, bigram:\t', ngram_lm.test_perplexity('/Users/gustavolucasdecarvalho/Desktop/a2/samiam.train','bigram'))
    print('Test perplexity, unigram:\t', ngram_lm.test_perplexity('/Users/gustavolucasdecarvalho/Desktop/a2/samiam.test', 'unigram'))
    print('Test perplexity, bigram:\t', ngram_lm.test_perplexity('/Users/gustavolucasdecarvalho/Desktop/a2/samiam.test','bigram'))
